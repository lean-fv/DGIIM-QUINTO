...
struct ResultMH {
  ...
  /// Constructs from a mutable solution reference (copies the solution).
  ResultMH(tSolution<tDomain> &sol, tFitness fit, unsigned evals)
      : solution(sol), fitness(fit), evaluations(evals) {}
  /// Constructs from a const solution reference (copies the solution).
  ResultMH(const tSolution<tDomain> &sol, tFitness fit, unsigned evals)
      : solution(sol), fitness(fit), evaluations(evals) {}
};
